...
- **2026-06-02 — Listener realtime de ofertas (bypass FCM) (v0.1.21):** plan B después de que el user confirmara que aún con todos los settings de batería del ZTE Nubia en orden, el FCM seguía fallando "la segunda vez en adelante". Nuevo `offer_realtime_listener.dart` que arranca al boot del app: mantiene un websocket Supabase abierto sobre `dispatch_offers` filtrado por `rider_id = self`. Cuando llega una INSERT con una oferta nueva, fetchea la orden y dispara `showCallkitIncoming` directo, sin pasar por FCM. La foreground service del geolocator (que se enciende al "Estoy en línea") mantiene el proceso Dart vivo en background → el websocket sigue conectado. El plugin callkit dedupea por `id=order_id` así que si el FCM también dispara (caso "anduvo igual"), no se muestra doble popup. FCM queda como red de seguridad pero el camino principal ahora es realtime.
- **2026-06-02 — DartPluginRegistrant en bg handler + reset status al abrir app (v0.1.20):** dos fixes. (1) `DartPluginRegistrant.ensureInitialized()` agregado al inicio de `_fcmBackgroundHandler` — sin esa linea los plugins (callkit, etc.) no tenian sus MethodChannels registrados en el bg isolate, asi que `showCallkitIncoming` fallaba silenciosamente con la app matada. Era el motivo del "popup anda solo la primera vez". (2) Revertimos el auto-resume del status: nuevo `_resetStatusOnLaunchProvider` en `rider_home` que en el primer mount setea el rider a `paused` si quedo `available` de la sesion anterior. El user pidio expresamente "el boton de linea lo tuviera que apretar cada vez a abro la aplicacion" — el GPS sigue sincronizandose con el status, pero ahora siempre arranca pausado.
- **2026-06-02 — GPS persistente con foreground service + auto-sync con status (v0.1.19):** fix del feedback "cada vez que pongo en segundo plano tengo que activar la ubicación". Tres cambios en `rider_gps_tracker`: (1) en Android usamos `AndroidSettings.foregroundNotificationConfig` para que el stream de GPS y los heartbeats sigan corriendo con la app cerrada / celu bloqueado — aparece una notif persistente "La 10 — en línea". (2) Sacamos el `Switch` manual del card de GPS — el tracker ahora se sincroniza automáticamente con `rider.status`: si está `available`, arranca; si no, se apaga. (3) Cuando reabrís la app, el tracker lee el status persistente de la DB y arranca solo — no hace falta volver a tocar "Estoy en línea". El botón "Estoy en línea" / "Pausar" sigue siendo el único toggle, y controla todo desde un solo lugar.
